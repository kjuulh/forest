...
impl PrepareCommand {
    pub async fn execute(&self, state: &State) -> anyhow::Result<()> {
        ...
        let Some(deployment_component) = project.dependencies.get("non", "deployment") else {
        ...
        let Some(ProjectValue::Bool(enabled)) = deployment_values.get("enabled") else {
            anyhow::bail!("non.deployment.enabled is not specified");
        };
        ...
        let deployment_output = project.path.join(".non").join("deployment");
        ...
        for deployment_item in deployment_items {
            ...
            let config_file_path = output_path.join("non").join("config.json");
            if let Some(parent) = config_file_path.parent() {
                tokio::fs::create_dir_all(parent)
                    .await
                    .context("non config file path")?;
            }
