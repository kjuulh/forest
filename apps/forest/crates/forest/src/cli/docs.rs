...
fn print_commands() {
    ...
    println!();
    println!("    forest run build");
    println!("        Build the component for deployment, by dispatching to the build");
    println!("        component the project depends on (forest-contrib/build-rust, -go,");
    println!("        or -docker). Compiles binaries for the configured platforms.");
    ...
    println!();
    println!("        Value syntax:");
    println!("          --key value          Literal string value");
    println!("          --key @-             Read value from stdin");
    println!("          --key @/path/to/file Read value from a file");
    println!("          --flag               Boolean flag (no value, sets true)");
    println!();
    ...
    println!("  Deployment");
    println!();
    println!("    forest release prepare [--set KEY=VALUE ...]");
    ...
    println!();
    println!("        --set overrides config values without modifying forest.cue.");
    println!("        Useful for CI to pin image tags: --set myorg/service.tag=abc123");
    println!();
    println!("        Two targets, told apart by whether the path names a component:");
    println!("          --set org/component.key=value  a component's config");
    println!("          --set config.key=value         the deployment config block,");
    println!("                                         which a forest/generic@1");
    println!("                                         destination receives directly");
    println!("        e.g. --set config.image_tag=main-601f507");
    println!();
    println!("    forest release create --env <env> [--set key=value ...]");
    println!("        Combined: prepare + annotate + release in one command.");
    println!("        This is the recommended command for most deployments.");
    println!("        --set overrides are recorded in the release annotation metadata.");
    println!();
    println!("    forest release annotate [options]");
    println!("        Upload deployment artifacts to the forest server.");
    println!("        Commit SHA and branch are auto-detected from git if not specified.");
    println!();
    println!("    forest release <slug> [--env <env>]");
    println!("        Execute a release to destinations.");
    println!();
...
fn print_components() {
    ...
    println!("      }}");
    println!();
    println!("      // dependencies and commands are optional — omit if not needed");
...
fn print_templates() {
    ...
    println!();
    println!("  Custom Tests");
    println!();
    println!("    is configured      True for any non-undefined, non-none value (including {{}})");
    println!();
    println!("    Use `is configured` instead of truthiness checks for optional config blocks,");
    println!("    because {{% if config.foo %}} is false for empty objects {{}}.");
    println!();
    println!("      {{%- if config.forage_postgresql is configured %}}");
    println!("      ... deploy postgresql resources ...");
    println!("      {{%- endif %}}");
}
...
fn print_deployment() {
    ...
    println!();
    println!("  Flux v2 Destination (forest/flux@1)");
    println!();
    println!("    For Flux GitOps deployments, the release flow is:");
    println!("      1. Manifests are committed to the GitOps repository");
    println!("      2. If `reconcile_url` is configured on the destination, a POST is sent");
    println!("         to the Flux Receiver webhook to trigger immediate reconciliation");
    println!("      3. Without a webhook, Flux polls the repo on its configured interval");
    println!();
...
fn print_config() {
    ...
    println!("    language: version: \"v0.16.1\"");
    println!("    deps: {{");
    ...
    println!();
    println!("    Generated by `forest run build`. Contains:");
...
fn print_shell() {
    ...
    println!();
    println!("  1. Interactive shells — `forest shell zsh|bash|fish`");
    println!();
    println!("     Add to your rc file:");
    println!();
    println!("         eval \"$(forest shell zsh)\"    # or: bash");
    println!("         forest shell fish | source     # fish (~/.config/fish/config.fish)");
    println!();
    println!("     This prepends the shim dir to PATH (idempotently — a POSIX `case`");
    println!("     guard for zsh/bash, a `contains` guard for fish, makes re-sourcing");
    println!("     harmless) and defines the interactive helper functions (e.g.");
    println!("     forest-tmp).");
    println!();
    ...
    println!("       fish  ~/.config/fish/conf.d/forest.fish");
    println!("                          (fish auto-sources conf.d for ALL fish shells,");
    println!("                          interactive or not — full coverage like .zshenv)");
    println!();
